...
// Remove error messages in real-time as user corrects inputs
function attachRealtimeValidation(input, errorField, conditionFn, message) {
  input.addEventListener('input', () => {
    if (conditionFn()) {
      errorField.textContent = "";
      input.classList.remove("invalid");
    }
    calculateTotals(); // recalc totals in case quantity/unitPrice changed
  });
}
...
quantityInput.addEventListener('input', () => {
  ...
  if (parseFloat(quantityInput.value) > 0) {
    errorFields.quantity.textContent = "";
...
unitPriceInput.addEventListener('input', () => {
  ...
  if (parseFloat(unitPriceInput.value) > 0) {
    errorFields.unitPrice.textContent = "";
...
addSalesForm.customerName.addEventListener('input', () => {
  if (addSalesForm.customerName.value.trim()) {
    errorFields.customerName.textContent = "";
    addSalesForm.customerName.classList.remove("invalid");
  }
});

addSalesForm.salesAgent.addEventListener('input', () => {
  if (addSalesForm.salesAgent.value.trim()) {
    errorFields.salesAgent.textContent = "";
    addSalesForm.salesAgent.classList.remove("invalid");
  }
});
...
addSalesForm.addEventListener('submit', function(e) {
  ...
  successMessage.textContent = `Success! Sale recorded for ${newSale.customerName}. Total Price: UGX ${newSale.totalPrice.toFixed(2)}, Transport Charge: UGX ${newSale.transportCharge.toFixed(2)}`;
